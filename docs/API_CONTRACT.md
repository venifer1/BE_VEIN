...
| Method | Path | 설명 |
|---|---|---|
| GET | `/signals` | `?type=ABC\|TOP\|IMALOL & market & timeframe & instrument_id & watchlist_only & status & near_only & active_only & cursor`. **`active_only=true`(R54)**면 DETECTED/NEAR_COMPLETION만(만료·무효 숨김). 기본 false(하위호환). 카드: type, market, instrument, timeframe, status, score(구조), **pattern_score(종합 Pattern Score, nullable — 미계산 시 null, R101)**, current_price, **c_target(ABC/TOP=C목표가·IMALOL=C예상가/projectedClose, R80)**, pivots 요약(0/A/B 일자), detected_at, freshness |
