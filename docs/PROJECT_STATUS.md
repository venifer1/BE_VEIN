...
3. **실사용 마찰 제거** — 실제로 며칠 써보면서 걸리는 것부터 (알림 노이즈? 신호 과다? 차트 조작감?). ✅ 스텁 폴백 가시화(R40), ✅ 신호 과다 → 홈 "오늘의 주목 신호" 상위 큐레이션(R41), ✅ 알림 노이즈 → 조용한 시간(R42), ✅ 알림 요약 → 다이제스트(R45), ✅ 알림 스풀링 → 조용한 시간 보류 후 방출(R46). 남은 것: 차트 조작감
