...
| **거시경제/시장국면** | **코드 0줄.** `macro`/`regime` 패키지 자체가 없음 |
...
1. **거시경제 / 시장국면 엔진** (R32 기획 완료, 코드 0)
   - 장단기 금리차(10Y-2Y, 10Y-3M) · M2 · DXY
   - **BULL / BEAR / RANGE / TRANSITION** 국면 판정
...
전략은 **① 도구 티어 과금 + ② 코인 신호 유료화**다. 핵심 원칙은
**"조언(패턴 신호)은 무료로 열고, 자동화·검증 도구에서 돈을 받는다"** — 법적 안전지대와
상업적 논리가 같은 방향을 가리킨다.

- 상세 설계(티어표 · 가격 역산 · V23 스키마 · 게이팅 지점 · 법적 체크리스트)는
  **[`MONETIZATION.md`](MONETIZATION.md)** 참조
