...
> **기준일:** 2026-09-13 (R88 통계 리프레시 · R181 재확인: 컨트롤러33·엔드포인트89·마이그V29 유지, 테스트 커버리지 추가)
...
| R201 | **API_CONTRACT 경로 드리프트 교정** (BE, 문서) — 내 정보 실제 경로 `/api/v1/users/me`인데 계약이 `/me`로 오기(→404 유발) 발견·교정. `/me/onboarding\|entitlements\|notification-prefs`는 정확 확인. 코드 무변경 |
...
| R196 | **데모 경제 캘린더(upcomingMacroEvents) 창/디데이 단위 테스트** (FE, 회귀 보호) — 창 필터·[1,90] 클램프·date=오늘+dday export+3. vitest 8파일 132개 그린(129→+3). 로직 무변경 |
| R195 | **데모 알림 다이제스트(buildNotificationDigest) 분류/집계 단위 테스트** (FE, 회귀 보호) — 카테고리 분류·시간창 필터·안읽음 집계 export+3. vitest 8파일 129개 그린(126→+3). 로직 무변경 |
| R194 | **데모 캔들 캐시(getCandles) 메모이제이션 단위 테스트** (FE, 회귀 보호) — 200개 반환·동일키 같은 참조·타임프레임 분리 +3. vitest 8파일 126개 그린(123→+3). 코드 무변경 |
| R193 | **데모 지표 요약(getIndicators) 관계식 단위 테스트** (FE, 회귀 보호) — 볼린저 중앙선=MA20·밴드 정렬·RSI 밴드·MACD 히스토그램 항등 +2. vitest 8파일 123개 그린(121→+2). 코드 무변경 |
| R192 | **데모 캔들 생성기(genCandles) 불변식 단위 테스트** (FE, 회귀 보호) — OHLC 유효성·연속성(open=직전close)·균등간격·시드 결정성 +4. vitest 8파일 121개 그린(117→+4). 코드 무변경 |
| R191 | **데모 신호 상세(getSignalDetail) 조회/유효기간/알고버전 단위 테스트** (FE, 회귀 보호) — 접두사 관용 조회·유효기간 탐지+30일·타입별 algorithm_version +3. vitest 8파일 117개 그린(114→+3). mockData 상세 getter 전수 커버. 코드 무변경 |
...
| R186 | **데모 테마 구성종목(getThemeConstituents) 조회 단위 테스트** (FE, 회귀 보호) — 테마 상세·미매핑 빈목록·미존재 null +3. vitest 8파일 102개 그린(99→+3). 코드 무변경 |
| R185 | **데모 전략 실행 히스토리(getStrategyHistory) 정렬/캡 단위 테스트** (FE, 회귀 보호) — run_at 내림차순+limit 캡+미존재 id 빈배열 +3. vitest 8파일 99개 그린(96→+3). 코드 무변경 |
| R184 | **웹푸시 구독 필드 정리(WebPushService normalize/trimToNull) 단위 테스트** (BE, 회귀 보호) — 필수 null→""·선택 빈값→null(+trim) private→static+신규 테스트 +2. ASCII 경로 gradle 그린. 로직 무변경 |
| R183 | **가입 메타데이터 정리 헬퍼(AuthService.trimTo) 단위 테스트** (BE, 회귀 보호) — strip→빈값 null→max 절단(유입추적 컬럼 방어) private→static+신규 테스트 +3. ASCII 경로 gradle 그린. 로직 무변경 |
| R182 | **수집 실패 오류코드 분류(IngestionService.errorCodeOf) 단위 테스트** (BE, 회귀 보호) — ApiException→코드명·그 외→클래스명 private→static+신규 테스트 +2. ASCII 경로 gradle 그린. 로직 무변경 |
| R181 | **PROJECT_STATUS 통계 드리프트 점검 + 테스트 커버리지 반영** (BE, 문서) — 컨트롤러33·엔드포인트89·마이그V29 실측 재확인(드리프트 없음), §2.2에 자동화 테스트 규모(BE 47파일·215/FE 8파일·96) 신규 반영. 코드 무변경 |
| R180 | **전체 스위트 통합 헬스체크 + 커버리지 성장 확인** (BE/FE, 마일스톤) — BE `gradle test --rerun-tasks` 그린(47파일·215 @Test, R160 대비 +14파일·+60), FE vitest 8파일 96개(R160 90→+6). 리팩터 3건 외 동작 무변경. 코드 무변경 |
| R179 | **주식 심볼 접미사 제거(EquityInstrumentSyncService.stripSuffix) 단위 테스트** (BE, 회귀 보호) — 005930.KS→005930 첫 점 절단(BRK.B→BRK 주의) private→static+신규 테스트 +3. ASCII 경로 gradle 그린. 로직 무변경 |
| R178 | **백테스트 horizon 파서 예외/관용 경로 보강** (BE, 회귀 보호) — parseHorizon 대소문자·공백 관용 + 잘못된 입력 거부(null·무매칭·0 이하) 기존 테스트에 +2. ASCII 경로 gradle 그린. 코드 무변경 |
| R177 | **신호 성과 월 버킷 헬퍼(SignalPerformanceService isMonthBucket/monthOf) 단위 테스트** (BE, 회귀 보호) — 버킷 판정·UTC yyyy-MM 산출(월 경계) private→static+기존 테스트에 +3. ASCII 경로 gradle 그린(4→7). 로직 무변경 |
| R176 | **Bearer 토큰 추출 분리 + 단위 테스트** (BE, 리팩터+회귀 보호) — JwtAuthFilter 인라인 헤더 파싱을 순수 `resolveBearerToken`으로 분리(동작 보존)+신규 테스트 +4(접두사·trim·대소문자·빈토큰). ASCII 경로 gradle 그린 |
| R175 | **공개 엔드포인트 레이트리밋 라우팅(PublicRateLimitFilter.ruleFor) 단위 테스트** (BE, 회귀 보호) — signup 정확매칭(시간당10)·/public/ 접두사(분당60)·미대상 null private→package-private+신규 테스트 +4. ASCII 경로 gradle 그린. 로직 무변경 |
| R174 | **주식 등락률 계산 추출 + 단위 테스트** (BE, 리팩터+회귀 보호) — equitySnapshot 루프 인라인식을 순수 `changePct(close,prevClose)`로 추출(동작 보존)+신규 테스트 +4. ASCII 경로 gradle 그린 |
| R173 | **공통 에러 봉투(ApiError) 빌더 단위 테스트** (BE, 회귀 보호) — 빈/누락 field_errors→null 접힘·code/message/traceId 전달 신규 테스트 +3. ASCII 경로 gradle 그린. 코드 무변경 |
| R172 | **공통 성공 응답 봉투(ApiResponse) 빌더 단위 테스트** (BE, 회귀 보호) — 부록 C-1 봉투 of/list의 freshness·nextCursor·unreadCount 슬롯 계약 신규 테스트 +4. ASCII 경로 gradle 그린. 코드 무변경 |
| R171 | **틱띄기 체결 방향 분류(ScalpSidecarClient isBuy/isAsk) 단위 테스트** (BE, 회귀 보호) — B*/BID·A*/S*/SELL 휴리스틱 private→static+기존 테스트에 +2. ASCII 경로 gradle 그린(3→5). 로직 무변경 |
| R170 | **시장 데이터 신선도 판정(MarketController.freshness) 단위 테스트** (BE, 회귀 보호) — FRESH/DELAYED 배지 결정(null·창 초과→DELAYED) private→static+신규 테스트 +3. ASCII 경로 gradle 그린. 로직 무변경 |
| R169 | **지수 히스토리 다운샘플링(MarketIndexService.downsample) 단위 테스트** (BE, 회귀 보호) — 균등간격+최신점 항상 포함 private→static+신규 테스트 +4(n≤max 항등·인덱스 산술·첫/끝 보존). ASCII 경로 gradle 그린. 로직 무변경 |
| R168 | **웹푸시(VAPID) 순수 헬퍼 단위 테스트** (FE, 회귀 보호) — notificationIds·urlBase64ToUint8Array(base64url 디코드)·subscriptionMatchesKey(키 회전 감지) 신규 web-notifications.test.ts +6. vitest 8파일 96개 그린(90→+6). 코드 무변경 |
| R167 | **TVL 조회 모드 정규화(TvlService normalizeMode/plain) 단위 테스트** (BE, 회귀 보호) — PROTOCOL/CHAIN 검증·plain표기 private→static+신규 테스트 +2. ASCII 경로 gradle 그린. 로직 무변경 |
| R166 | **파생 목록 정렬 보조(DerivativesService priorityIndex/plain/num) 단위 테스트** (BE, 회귀 보호) — 메이저 우선순위·plain표기·안전파싱(무효→-inf) private→static+신규 테스트 +3. ASCII 경로 gradle 그린. 로직 무변경 |
| R165 | **김치 프리미엄 계산 추출 + 단위 테스트** (BE, 리팩터+회귀 보호) — refresh 루프 인라인식을 순수 `premiumPct(upbit,binance,usdkrw)`로 추출(동작 보존)+신규 테스트 +5(정/역프·패리티·반올림·null). ASCII 경로 gradle 그린 |
| R164 | **속보 조회 파라미터 정규화(NewsService normalizeSource/Symbol/clampSize) 단위 테스트** (BE, 회귀 보호) — source검증·symbol대문자·페이지크기 [1,100] 클램프 private→static+신규 테스트 +3. ASCII 경로 gradle 그린. 로직 무변경 |
| R163 | **모의투자 주문 입력 정규화(PaperTradingService normalize*·leverage) 단위 테스트** (BE, 회귀 보호) — side/type/투자유형/포지션방향 기본값·검증 + 레버리지 50x캡 private→static+기존 테스트 +5(400 누출 방지). ASCII 경로 gradle 그린(4→9). 로직 무변경 |
| R162 | **공개 캔들 DTO 매핑(CandleDto.from) 단위 테스트** (BE, 회귀 보호) — BigDecimal→plain string(지수표기 방지)·null 안전·openTime ISO 위임 신규 테스트 +3. ASCII 경로 gradle 그린. 코드 무변경 |
| R161 | **신호 멱등키(PatternSignal.buildSignalKey) 단위 테스트** (BE, 회귀 보호) — §10.1 중복방지 키(signal_key UNIQUE 직결) 포맷·결정성·필드별 구분성 신규 테스트 +3. ASCII 경로 gradle 그린. 코드 무변경 |
| R160 | **전체 스위트 통합 헬스체크 + 커버리지 성장 확인** (BE/FE, 마일스톤) — BE 전체 gradle test 그린(33파일·155 @Test, R143 대비 +5파일·+28), FE vitest 7파일 90개(R143 47→+43). 코드 무변경 |
| R159 | **Explain 거래량비율(SignalExplainService.volumeRatio) 단위 테스트** (BE, 회귀 보호) — 최신봉÷나머지평균(2자리, 조건검색기 4자리와 별개) private→static+기존 테스트에 +4. ASCII 경로 gradle 그린(4→8). 로직 무변경 |
| R158 | **조건검색 거래량비율(ConditionScannerService.volumeRatio) 단위 테스트** (BE, 회귀 보호) — 최신봉÷직전20봉평균 private→static+기존 테스트에 +4(20봉창상한·null·반올림). ASCII 경로 gradle 그린(7→11). 로직 무변경 |
| R157 | **평균 고저폭 변동성(SignalExplainService.averageRangePct) 단위 테스트** (BE, 회귀 보호) — `avg((high-low)/close*100)` private→static+신규 테스트 +4(평균·close0제외·null·반올림). ASCII 경로 gradle 그린. 로직 무변경 |
| R156 | **모의투자 금액/수량/수익률 표기(PaperTradingService) 단위 테스트** (BE, 회귀 보호) — money/qty(8자리)·pct(4자리) HALF_UP+후행0제거 private→static+신규 테스트 +4(0→"0" 확인). ASCII 경로 gradle 그린. 로직 무변경 |
| R155 | **틱띄기 매수/매도 비율(ScalpSidecarClient.ratio) 단위 테스트** (BE, 회귀 보호) — part/total 4자리 HALF_UP 표기 private→static+신규 테스트 +3. ASCII 경로 gradle 그린. 로직 무변경 |
| R154 | **신호 성과 수익률(SignalPerformanceService.pct) 단위 테스트** (BE, 회귀 보호) — return_pct·MFE·MAE 공유 공식 `(value/base-1)*100` private→static+신규 테스트 +4(상승/하락·반올림·null). ASCII 경로 gradle 그린. 로직 무변경 |
| R153 | **유통량 비율(SupplyService.circulatingPct) 단위 테스트** (BE, 회귀 보호) — max우선·total폴백·null 케이스 신규 SupplyServiceTest +5. ASCII 경로 gradle test 그린. 로직 무변경 |
| R152 | **데모 백테스트 파이프라인(runBacktest) 결정성·불변식 테스트** (FE, 회귀 보호) — 시드 결정성(metrics·수익률 동일)·구조(체결28~35·equity=체결+1·best≥worst)·파라미터 에코·워크포워드 is_ratio 클램프 +4. vitest 7파일 90개 그린(86→+4). 코드 무변경 |
| R151 | **데모 공포·탐욕 히스토리 생성기 불변식 테스트** (FE, 회귀 보호) — `getFearGreedHistory`(시드 시계열) 길이·[5,95]클램프·시드결정성·날짜정렬 +4케이스. vitest 7파일 86개 그린(82→+4). 코드 무변경 |
| R150 | **데모 어댑터 요청 파싱(parseUrl·body) 단위 테스트** (FE, 회귀 보호) — 전 라우트 진입 파서(경로/쿼리 분리·params 병합·본문 JSON 파싱) export+7케이스. vitest 7파일 82개 그린(75→+7). 로직 무변경 |
| R149 | **데모 딥링크 경로 id 매칭(idMatches) 단위 테스트** (FE, 회귀 보호) — `usr_2`↔`2` 접두사 완화 매칭 규칙(첫 alpha_만 제거·대소문자 구분) export+5케이스. vitest 7파일 75개 그린(70→+5). 로직 무변경 |
| R148 | **데모 어댑터 순수 헬퍼(초성검색·숫자표기) 단위 테스트** (FE, 회귀 보호) — `mockAdapter`의 `toCho`·`isChoQuery`·`num`·`trimNum` export+신규 `mockAdapter.test.ts`(+6). node env import 무부작용 확인. vitest 7파일 70개 그린(64→+6). 로직 무변경 |
| R147 | **등락 순위(movers) 라우팅+정렬 단위 테스트** (FE, 회귀 보호) — `getMovers` 4시장×3타입 분기·VOLUME 거래대금 내림차순 불변식 +4케이스(데이터 아닌 불변식 단언). vitest 64개 그린(60→+4). 코드 무변경 |
| R146 | **데모 백테스트 지표 집계 단위 테스트** (FE, 회귀 보호) — `metricsFromTrades`(승률·손익비·복리수익·MDD) export+4케이스(대표·손익비99상한·빈목록·NaN제외+null→0%). vitest 60개 그린(56→+4). 로직 무변경, null→0 동작 못박음 |
| R145 | **공포·탐욕 지수 라벨 분류 경계값 단위 테스트** (FE, 회귀 보호) — 데모 F&G 히스토리 `classifyFg`(0~100→5단계) 무테스트 갭. export+경계 10단언(24/44/55/74 전이). vitest 56개 그린(55→+1). 로직 무변경 |
| R144 | **차트 오버레이 지표(SMA·볼린저) 순수 계산 단위 테스트** (FE, 회귀 보호) — `chart-view` 오버레이가 쓰는 `lib/indicators.ts` 무테스트 갭 해소. `indicators.test.ts`+8(선행 null 정렬·상수열 sd0 수렴·모집단 sd). vitest 6파일 55개 그린(47→+8). 코드 무변경 |
| R143 | **전체 테스트 스위트 통합 그린 + 커버리지 성장 확인** (BE/FE, 헬스체크) — BE 전체 `gradlew test` 그린(28파일·127 @Test, R116 대비 +6파일·+29). FE vitest 47. 코드 무변경 |
